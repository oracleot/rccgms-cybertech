...
export const magicLinkSchema = z.object({
  email: z.string().email("Please enter a valid email address"),
  // Where to land after authenticating. Canonical name is `next`; the value
  // is untrusted until it goes through sanitizeNext() (lib/auth/next-url.ts),
  // so no format validation belongs here.
  next: z.string().optional(),
  /** @deprecated legacy name for `next` — still accepted, never written. */

...
export function LiveView({ rundownId, items, serviceName, itemsWithSongs }: LiveViewProps) {
  ...
  // Navigate directly to a specific item (e.g. from sidebar)
  const handleGoToItem = useCallback(
    (index: number) => {
      ...
      // Ensure any transition state on connected displays is cleared
      sendMessage({
        type: "TRANSITION",
        payload: {
    ...
    [orderedItems.length, sendMessage, serviceName]
  )
      ...
      <div className="flex items-center justify-between flex-wrap gap-3">
        ...
        <div className="flex items-center gap-3 flex-wrap">
          ...
          {started && !isInTransition && currentIndex > 0 && (
            <Button variant="outline" size="sm" onClick={() => handleGoToItem(currentIndex - 1)}>
          ...
          <CardContent className="space-y-2 max-h-[260px] overflow-y-auto">
            {orderedItems.map((item, idx) => (
              <div
                key={item.id}
                className={cn(
                  "flex items-center justify-between rounded-md border px-3 py-2 text-sm cursor-pointer hover:bg-muted/60 transition-colors",

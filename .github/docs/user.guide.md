# Cyber Tech User Guide

> Your guide to using the Cyber Tech app for managing our church's tech department

Welcome to Cyber Tech! This guide will help you get started and make the most of the app. Whether you're a volunteer checking your schedule or a leader managing the team, you'll find everything you need here.

---

## Quick Start

**New to Cyber Tech?** Here's how to get going in under 2 minutes:

1. **Check your email** for an invitation from your team leader
2. **Click the invitation link** to set up your password
3. **Sign in** at the login page
4. **View your dashboard** to see upcoming duties

That's it! You're ready to go. 🎉

---

## Getting Started

### Creating Your Account

Most users will receive an invitation email from a team leader or admin. Here's how to get set up:

1. Look for an email with the subject "You're invited to Cyber Tech"
2. Click the **Accept Invitation** button in the email
3. You'll be taken to a page to create your password
4. Enter a strong password (at least 8 characters)
5. Confirm your password by typing it again
6. Click **Set Password**

You'll be automatically signed in and taken to your dashboard.

> 💡 **Tip**: Use a password you'll remember, but make sure it's secure. A mix of letters, numbers, and symbols works best.

### Signing Up (If Self-Registration is Enabled)

If your church allows self-registration:

1. Go to the Cyber Tech app
2. Click **Create an account** below the sign-in form
3. Enter your full name
4. Enter your email address
5. Create a password (at least 8 characters)
6. Confirm your password
7. Click **Create account**
8. Check your email for a verification link
9. Click the link to verify your email

After verification, you can sign in to your account.

---

## Signing In

### How To Sign In

1. Open the Cyber Tech app in your browser
2. Enter your **email address**
3. Enter your **password**
4. Click **Sign in**

You'll be taken to your dashboard where you can see your upcoming duties.

### Forgot Your Password?

Don't worry – it happens to everyone! Here's how to reset it:

1. On the sign-in page, click **Forgot password?**
2. Enter the email address you use for Cyber Tech
3. Click **Send Reset Link**
4. Check your email for a message from Cyber Tech
5. Click the **Reset Password** link in the email
6. Enter your new password
7. Confirm your new password
8. Click **Update Password**

You'll see a success message and can sign in with your new password.

### Tips & Tricks

- ✅ Bookmark the app for quick access on Sundays
- ✅ Save the login page to your phone's home screen
- ❌ Don't share your password with others – each person should have their own account

### Common Questions

**Q: I didn't receive the password reset email?**  
A: Check your spam or junk folder. If it's still not there, wait a few minutes and try again. If problems persist, contact your team leader.

**Q: It says my email isn't registered?**  
A: You may not have been invited yet. Ask your team leader to send you an invitation.

**Q: I'm getting an "invalid credentials" error?**  
A: Double-check that you're using the correct email and password. Remember that passwords are case-sensitive. Try resetting your password if you're unsure.

---

## Your Profile & Settings

### Accessing Settings

1. Sign in to Cyber Tech
2. Click on your **profile picture** or **initials** in the top-right corner
3. Select **Settings** from the menu

### Updating Your Profile

Keep your information up to date so your team can reach you:

1. Go to **Settings**
2. In the **Profile** section, you can update:
   - **Name** – How you want to appear in the app
   - **Phone number** – For SMS notifications (optional)
   - **Department** – Which team you're part of
   - **Profile picture** – Click on your avatar to upload a new photo

3. Click **Save changes** when you're done

### Changing Your Profile Picture

1. Go to **Settings**
2. Click on your current profile picture (or the placeholder with your initials)
3. Select an image from your device (JPEG, PNG, or WebP)
4. The image must be under 5MB
5. Your new picture will upload automatically

### Notification Preferences

Choose how you want to be notified about your duties and updates:

1. Go to **Settings**
2. Scroll down to **Notification Preferences**
3. For each notification type, you can toggle:
   - **Email** – Receive notifications via email
   - **SMS** – Receive text messages (requires phone number)

4. For **Rota Reminders**, you can also choose when to receive them:
   - 1 day before
   - 2 days before
   - 1 week before

5. Click **Save preferences** when you're done

#### Notification Types

| Notification | What it's for |
|-------------|---------------|
| **Rota Published** | When a new schedule is posted |
| **Rota Reminder** | Reminder before you're scheduled to serve |
| **Swap Request** | When someone wants to swap duties with you |
| **Swap Approved** | When a swap request is approved |
| **Training Updates** | Updates about your training progress |

### Tips & Tricks

- ✅ Add your phone number to receive SMS reminders
- ✅ Enable rota reminders so you never forget when you're serving
- ✅ Upload a profile picture so your team can recognize you

---

## Understanding Your Role

Cyber Tech has three user roles with different levels of access:

### 👤 Volunteer

**What you can do:**
- View your schedule and upcoming duties
- Update your availability
- Request duty swaps with other volunteers
- Update your profile and notification preferences
- Track your training progress

### 👥 Leader

**Everything a Volunteer can do, plus:**
- Create and publish rotas (schedules)
- Assign volunteers to positions
- Approve or decline swap requests
- View team availability
- Manage equipment checkouts
- Create service rundowns
- Generate livestream descriptions

### 🔧 Admin

**Everything a Leader can do, plus:**
- Invite new users to the system
- Manage user roles and permissions
- Configure departments and positions
- Access all system settings

---

## 🔧 For Admins: Inviting New Users

As an admin, you can invite new members to join Cyber Tech:

1. The invite feature is accessed through your admin panel
2. Enter the new user's email address
3. Select their role (Volunteer, Leader, or Admin)
4. Choose their department
5. Click **Send Invitation**

The new user will receive an email with instructions to set up their account.

> 💡 **Tip**: Most new users should start as Volunteers. You can always upgrade their role later.

---

## Need Help?

If you're having trouble with the app:

1. **Check this guide** – Your answer might be here!
2. **Ask your team leader** – They can help with most issues
3. **Contact your admin** – For account or access problems

---

## 📅 Availability Management

Setting your availability helps leaders know when you're able to serve. By marking dates as available or unavailable in advance, you help the team create better schedules.

### What You Can Do

**👤 Volunteers:**
- View your availability calendar
- Mark specific dates as available or unavailable
- Add notes explaining why you're unavailable
- Bulk-select multiple dates (like all Sundays in a month)

**👥 Leaders & 🔧 Admins:**
- Everything volunteers can do, plus:
- View team availability across all volunteers
- Filter by department to see specific team availability
- Use availability info when assigning volunteers to rotas

---

### Setting Your Availability

1. Go to **Rota** from the main menu
2. Click **My Schedule** in the navigation
3. Click the **Set Availability** button, or go directly to **Rota → Availability**

You'll see a calendar where you can mark your availability.

---

### How to Mark Dates

#### Selecting Dates

1. **Click on dates** in the calendar to select them
   - Selected dates will have a blue highlight
   - Click again to deselect
2. You can select multiple dates at once
3. The number of selected dates shows at the top

#### Quick Selection Options

Save time with these quick actions:

- **Select All Sundays** – Selects every Sunday in the current month
- **Next 4 Sundays** – Selects the upcoming four Sundays

#### Marking as Available or Unavailable

Once you've selected dates:

1. Click **Mark Available** (green button) to indicate you can serve
2. Click **Mark Unavailable** (red button) to indicate you cannot serve
3. If marking unavailable, you can add an optional note (e.g., "Family holiday", "Traveling")
4. Click **Confirm** to save

#### Understanding the Calendar

The calendar uses color coding:
- 🟢 **Green dot** = You're available on this date
- 🔴 **Red dot** = You're unavailable on this date
- **No dot** = You haven't set your availability yet

---

### Clearing Your Availability

Changed your plans? You can clear your availability:

1. Go to **Rota → Availability**
2. Select the date(s) you want to change
3. Click **Clear** to remove your availability setting
4. The date will return to "not set" status

---

### 👥 Viewing Team Availability (Leaders & Admins)

Leaders can see everyone's availability when planning rotas:

1. Go to **Rota** from the main menu
2. Click **View Team Availability** (or go to **Rota → Team Availability**)

You'll see a grid showing:
- All team members in rows
- Days of the week in columns
- ✅ Green = Available
- ❌ Red = Unavailable
- ➖ Gray = Not set

#### Features

- **Week Navigation** – Use arrows to move between weeks
- **Today Button** – Jump back to the current week
- **Department Filter** – Show only specific teams (Sound, Cameras, etc.)
- **Summary Row** – Shows counts of available/unavailable per day
- **Hover for Details** – Hover over any cell to see notes

#### Sunday Summary

At the bottom, you'll see a quick summary for Sundays showing:
- Number of available volunteers
- Percentage of team available
- Color-coded badges (green = 75%+, yellow = 50-75%, red = <50%)

---

### Availability in Volunteer Selection

When leaders assign volunteers to positions, they'll see availability:

- **"Available" badge** (green) – Volunteer has marked themselves available
- **"Unavailable" badge** (red) – Volunteer has marked themselves unavailable
- **No badge** – Volunteer hasn't set availability for this date

> ⚠️ **Note for Leaders**: Volunteers marked as unavailable appear dimmed but can still be selected if absolutely necessary. Try to respect availability preferences when possible!

---

### Availability Tips & Best Practices

**For Volunteers:**
- ✅ Set your availability at least 2 weeks in advance
- ✅ Use the "Next 4 Sundays" quick action for easy planning
- ✅ Add notes when you're unavailable – it helps leaders understand
- ✅ Update immediately if your plans change
- ❌ Don't mark yourself unavailable just to avoid serving – your team needs you!

**For Leaders:**
- ✅ Check team availability before creating rotas
- ✅ Use the department filter to see relevant team members
- ✅ Hover over "Unavailable" cells to see notes
- ✅ Contact volunteers with no availability set to remind them
- ✅ Respect unavailability when possible – it builds team trust

---

### Common Questions

**Q: Do I need to mark every date?**  
A: No, you only need to mark dates where you know you're unavailable. If you don't mark a date, leaders will assume you might be available.

**Q: Can leaders still schedule me if I'm marked unavailable?**  
A: Technically yes, but leaders should respect your availability. If you're scheduled on a day you marked unavailable, contact your leader to discuss.

**Q: How far in advance can I set availability?**  
A: You can set availability for any future date. We recommend setting at least a month ahead when you know your schedule.

**Q: Can I add notes when marking myself available?**  
A: Currently, notes are only available when marking unavailable. For available dates, no note is needed.

**Q: Will I get a reminder to update my availability?**  
A: Not yet, but this feature is coming soon! For now, set a personal reminder to update your availability each month.

---

## 📅 Rota Management

The rota is your service schedule – it shows who's serving in which position for each service. This is where you'll find out when you're on duty.

### What You Can Do

**👤 Volunteers:**
- View the service schedule in calendar or list view
- See your personal upcoming duties
- Check who else is serving on any given Sunday

**👥 Leaders & 🔧 Admins:**
- Create new rotas for upcoming services
- Assign volunteers to positions
- Publish rotas to notify the team
- Edit or delete draft rotas

---

### Viewing the Rota Schedule

There are two ways to view the schedule:

#### Calendar View

The calendar shows all services at a glance, color-coded by status:
- 🟢 **Green** = Published (final schedule)
- 🟠 **Amber** = Draft (still being worked on by leaders)

**How to use the calendar:**
1. Go to **Rota** from the main menu
2. You'll see the calendar view by default
3. Click the arrows to navigate between months
4. Click on any service to see the full details

#### List View

Prefer a simple list? Switch to list view:

1. Go to **Rota** from the main menu
2. Click the **List** tab at the top
3. You'll see upcoming services in chronological order
4. Click on any row to view that rota's details

---

### 📋 My Schedule – Your Personal Duties

The easiest way to see when you're serving:

1. Go to **Rota** from the main menu
2. Click **My Schedule** in the top navigation
3. You'll see all your upcoming assignments

Each assignment shows:
- 📅 The date and service name
- 🎯 Your position (e.g., Camera 1, Sound Desk)
- 🏷️ Your current status (Pending, Confirmed, or Declined)
- 🕐 Service time and location

> 💡 **Tip**: Check your schedule at the start of each week so you know when you're needed!

---

### Viewing Rota Details

To see the full team for any service:

1. Click on a service from the calendar or list
2. You'll see the **Rota Details** page showing:
   - Service name and date
   - All positions grouped by department
   - Who's assigned to each position
   - The rota status (Draft or Published)

This is helpful for seeing who you'll be serving with and what positions still need to be filled.

---

### 👥 Creating a New Rota (Leaders & Admins)

Ready to schedule your team for an upcoming service?

1. Go to **Rota** from the main menu
2. Click the **+ New Rota** button (top right)
3. **Select the service** from the dropdown (e.g., "Sunday Service")
4. **Choose the date** using the date picker
5. Click **Create Rota**

You'll be taken to the rota detail page where you can start assigning volunteers.

> 💡 **Tip**: You can also click on any empty date in the calendar view to start a new rota for that date.

---

### 👥 Assigning Volunteers to Positions (Leaders & Admins)

Once you've created a rota:

1. Open the rota detail page
2. You'll see all positions organized by department
3. For each position, click on **Select volunteer**
4. Choose a team member from the dropdown
5. Repeat for all positions you want to fill

**Helpful features:**
- Volunteers are shown with their availability status
- Names marked as "Unavailable" have indicated they can't serve that day
- Use the search box to quickly find team members
- You can drag and drop to rearrange positions

> ⚠️ **Remember**: Volunteers won't see the schedule until you publish the rota!

---

### 👥 Publishing a Rota (Leaders & Admins)

When you're ready to share the schedule with the team:

1. Open the rota you want to publish
2. Review that all key positions are filled
3. Click the **Publish** button
4. Confirm by clicking **Yes, Publish**

**What happens when you publish:**
- 📧 All assigned volunteers receive an email notification
- 📅 The rota appears on everyone's "My Schedule" page
- 🟢 The calendar shows the service in green (published)
- 🔒 The schedule is locked (no more changes without unpublishing)

> 💡 **Tip**: Double-check your assignments before publishing – volunteers will be notified immediately!

---

### 👥 Editing or Deleting a Rota (Leaders & Admins)

**Editing a draft rota:**
1. Click on the rota from the calendar or list
2. Make your changes to assignments
3. Changes are saved automatically

**Deleting a draft rota:**
1. Click on the rota to open it
2. Click the **Delete** button (trash icon)
3. Confirm the deletion

> ⚠️ **Note**: You can only delete rotas that haven't been published yet. Once published, the rota is locked to prevent confusion.

---

### Upcoming Assignments Widget

On your dashboard, you'll see the **Upcoming Assignments** card showing your next few duties. This gives you a quick glance at when you're serving without navigating to the full schedule.

Each item shows:
- The date in a bold calendar format
- Your position
- The service name
- Your confirmation status

Click **View all** to see your complete schedule.

---

### Rota Tips & Best Practices

**For Volunteers:**
- ✅ Check your schedule every week
- ✅ Enable email notifications so you don't miss assignments
- ✅ Submit your availability in advance (coming soon!)
- ✅ Look at who else is on your team – teamwork makes the dream work!

**For Leaders:**
- ✅ Create rotas at least 2 weeks in advance
- ✅ Check volunteer availability before assigning
- ✅ Publish rotas early so the team can plan
- ✅ Fill critical positions first (Sound, Camera Lead, etc.)
- ❌ Don't assign the same person to every service – spread the load

---

### Common Questions

**Q: I don't see any rotas on the calendar?**  
A: There may not be any published rotas yet for upcoming services. If you're a volunteer, you'll only see published schedules. Leaders and admins can also see drafts.

**Q: Why can't I see the "New Rota" button?**  
A: Only Leaders and Admins can create rotas. If you need to create schedules, ask an Admin to upgrade your role.

**Q: I was assigned but didn't receive an email?**  
A: Check your spam folder. Also, make sure email notifications are enabled in your Settings → Notification Preferences.

**Q: How do I change my assignment?**  
A: Volunteers can't change their own assignments directly. You can request a swap with another volunteer (coming soon!) or contact your team leader.

**Q: Can I see past rotas?**  
A: Yes! Use the calendar arrows to navigate to previous months, or check the list view for past services.

---

## Coming Soon

We're working on adding more features to Cyber Tech:

-  **Duty Swap Requests** – Request to swap duties with another volunteer
- 🎥 **Livestream Tools** – Generate descriptions for YouTube and Facebook
- 📋 **Service Rundowns** – Create and manage service order of events
- 📦 **Equipment Tracking** – Check out and manage tech equipment
- 📚 **Training** – Complete onboarding and training modules

Stay tuned for updates!

---

*Last updated: December 2025*
